...
import com.alibaba.nacos.plugin.datasource.impl.dialect.DamengDatabaseDialect;
...
class DamengMapperCoverageTest {
    ...
    @Test
    void testDamengDatabaseDialects() {
        DamengDatabaseDialect damengDialect = new DamengDatabaseDialect();
        assertEquals(DatabaseTypeConstant.DM, damengDialect.getType());
        assertEquals("SYSDATE", damengDialect.getFunction("NOW()"));
        assertArrayEquals(PrimaryKeyConstant.UPPER_RETURN_PRIMARY_KEYS,
            damengDialect.getReturnPrimaryKeys());
    }

    @Test
    void testConfigInfoMapperOptionalBranches() {
        ConfigInfoMapperByDameng mapper = new ConfigInfoMapperByDameng();
        assertEquals(DatabaseTypeConstant.DM, mapper.getDataSource());
        assertEquals("SYSDATE", mapper.getFunction("NOW()"));
        assertArrayEquals(PrimaryKeyConstant.UPPER_RETURN_PRIMARY_KEYS,
            mapper.getPrimaryKeyGeneratedKeys());
        ...
        assertResult(mapper.findConfigInfoLike4PageFetchRows(context),
            "SELECT a.id,a.data_id,a.group_id,a.tenant_id,a.app_name,a.content,a.md5,"
                ...
                + "LISTAGG(b.tag_name, ',') WITHIN GROUP (ORDER BY b.tag_name) as config_tags FROM "
    ...
    void testConfigTagsRelationMapperTypeFilter() {
        ConfigTagsRelationMapperByDameng mapper = new ConfigTagsRelationMapperByDameng();
        ...
        assertResult(mapper.findConfigInfoLike4PageFetchRows(context),
            "SELECT c.id,c.data_id,c.group_id,c.tenant_id,c.app_name,c.content,c.md5,"
                ...
                + "LISTAGG(DISTINCT d.tag_name, ',') WITHIN GROUP (ORDER BY d.tag_name) as config_tags FROM "
    ...
    void testHistoryAndGrayAndMigrateMappers() {
        HistoryConfigInfoMapperByDameng historyMapper = new HistoryConfigInfoMapperByDameng();
        assertEquals(DatabaseTypeConstant.DM, historyMapper.getDataSource());
        assertEquals("SYSDATE", historyMapper.getFunction("NOW()"));
        assertResult(historyMapper.removeConfigHistory(context),
            "DELETE FROM his_config_info WHERE ROWID IN (SELECT ROWID FROM his_config_info WHERE gmt_modified < ? LIMIT ?)", startTime, 20);
        ...
        ConfigInfoGrayMapperByDameng grayMapper = new ConfigInfoGrayMapperByDameng();
        assertEquals(DatabaseTypeConstant.DM, grayMapper.getDataSource());
        assertEquals("SYSDATE", grayMapper.getFunction("NOW()"));
        ...
        ConfigMigrateMapperByDameng migrateMapper = new ConfigMigrateMapperByDameng();
        assertEquals(DatabaseTypeConstant.DM, migrateMapper.getDataSource());
        assertEquals("SYSDATE", migrateMapper.getFunction("NOW()"));
        assertEquals(TableConstant.MIGRATE_CONFIG, migrateMapper.getTableName());
    }

    @Test
    void testCapacityMapperDataSource() {
        TenantCapacityMapperByDameng tenantCapacityMapper = new TenantCapacityMapperByDameng();
        assertEquals(DatabaseTypeConstant.DM, tenantCapacityMapper.getDataSource());
        ...
        GroupCapacityMapperByDameng groupCapacityMapper = new GroupCapacityMapperByDameng();
        assertEquals(DatabaseTypeConstant.DM, groupCapacityMapper.getDataSource());
    ...
    void testAiResourceMapper() {
        AiResourceMapperByDameng mapper = new AiResourceMapperByDameng();
        assertEquals(DatabaseTypeConstant.DM, mapper.getDataSource());
        assertEquals("SYSDATE", mapper.getFunction("NOW()"));
    ...
    void testAiResourceVersionMapper() {
        AiResourceVersionMapperByDameng mapper = new AiResourceVersionMapperByDameng();
        assertEquals(DatabaseTypeConstant.DM, mapper.getDataSource());
        assertEquals("SYSDATE", mapper.getFunction("NOW()"));
